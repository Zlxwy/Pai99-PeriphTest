// Directly map the error to the control variable for control
// 直接将误差映射至控制量进行控制
// 按下空格键启动停止
...
int test_ocv_ctrl_car_directly(void) {
    /*键盘对象，用于按下ESC键后结束程序*/
    ...
    std::shared_ptr<PwmDef> motorPwm = std::make_shared<PwmDef>("pwmchip2", "0", MOTOR_CNT_MAX, speedPercent2cnt(0, MOTOR_CNT_MAX), "inversed", motorEn);
    ...
    for(auto &dir: motorDir) dir->setVal(1); // 前进方向
    ...
    while (true) { // 开始循环
        ...
        /********************根据计算出来的斜率，控制硬件驱动****************/
        // 电机设置前进方向
        for(auto &dir: motorDir) dir->setVal(1); //设置电机转动方向：前进
        motorPwm->setDuty( speedPercent2cnt(15, MOTOR_CNT_MAX) ); //设置电机转动速度（高电平占空比25%）
        // 将斜率限制在[xlMIN,xlMAX](右到左)之间，映射至角度[SERVO_ANGLE_MIN,SERVO_ANGLE_MAX](右到左)
        constexpr double xlMIN = -0.7; // 斜率范围最小值
        constexpr double xlMAX = +0.7; // 斜率范围最大值
        xl = myClamp(xl, xlMIN, xlMAX); // 限制斜率的范围
        double angle = mapArea(xlMIN, xlMAX, SERVO_ANGLE_MIN, SERVO_ANGLE_MAX, xl); // 把斜率值映射至舵机转角值
        angle = myClamp(angle, SERVO_ANGLE_MIN, SERVO_ANGLE_MAX); // 限制舵机转角值的范围
        servoPwm->setDuty( angle2cnt(angle, SERVO_CNT_MAX) ); // 设置舵机角度
        ...
        std::cout << "角度：" << angle << " (" << angle2cnt(angle, SERVO_CNT_MAX) << ")" << std::endl;
