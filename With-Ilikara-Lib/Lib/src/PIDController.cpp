...
constexpr T clamp(T value, T min, T max) {
    static_assert(std::is_arithmetic<T>::value, "clamp: T must be an arithmetic type");
    // 使用static_assert确保模板参数T是一个算术类型（整数或浮点数）
    if (min > max) { // 如果最小值大于最大值，交换它们
        min = min + max;
        max = min - max;
        min = min - max;
    }
...
double PIDController::positionPID(double error)
{
    ...
    // 输出饱和处理
    output = clamp(output, -output_limit_, output_limit_);
...
double PIDController::incrementalPID(double error)
{
    ...
    // 输出饱和处理
    prev_output_ = clamp(prev_output_, -output_limit_, output_limit_);
