...
// 按下 a 键靠左行驶，按下 d 键靠右行驶，按下 s 键恢复为靠中行驶，按 w 启停直走
// （偷偷用键盘遥控的）
        ...
        if ( kb.kbhit() ) {
            ...
            switch (getKey) {
                case KEY_SPACE: // 启动/停止电机
                    ...
                    shouldPlaySound += 1;
                    switch (shouldPlaySound) { // 控制声音播放
                        case 2: case 4: // 播放声音
                            wonderEchoSend(0xff, 0x10); // 播放“人行道前停车礼让行人”的声音
                            break;
                        default: break;
                    } // 结束声音播放控制
                    // 不用加break，还需要执行下面的case
                case KEY_S: case KEY_s: case KEY_W: case KEY_w: // 恢复为靠中行驶
                    xlPID.targetVal = 0.0;
                    pyPID.targetVal = (0+RESIZED_WIDTH-1) / 2.0;
                    break;
                case KEY_A: case KEY_a: // 变化为靠左行驶
                    xlPID.targetVal = 0.0 - XL_OFFSET;
                    pyPID.targetVal = (0+RESIZED_WIDTH-1) / 2.0 - PY_OFFSET;
                    break;
                case KEY_D: case KEY_d: // 变化为靠右行驶
                    xlPID.targetVal = 0.0 + XL_OFFSET;
                    pyPID.targetVal = (0+RESIZED_WIDTH-1) / 2.0 + PY_OFFSET;
                    break;
                default: break;
